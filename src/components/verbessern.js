...
const Verbessern = (props) => {
  return (
    <div className={`verbessern-feature-card ${props.rootClassName} `}>
      <svg viewBox="0 0 1024 1024" className="verbessern-icon">
        <path
      ...
      <h2 className="verbessern-text">{props.title}</h2>
      <form className="verbessern-form">
        <input
          type="text"
          required
        ...
        />
        <button type="button" className="verbessern-button button">
          {props.button}
        </button>
      </form>
      <span className="verbessern-text1">{props.description}</span>
    </div>
  )
}

Verbessern.defaultProps = {
  description: 'Gerichte auf ein neues Level bringen',
  title: 'Verbessern',
  rootClassName: '',
  button: 'Vebessern',
  action: 'SEE MORE',
